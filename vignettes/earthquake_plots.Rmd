...
plot like the one shown below. Date is represented on the x-axis, 
Country is represented on the y-axis, point size reflects the magnitude 
(based on the field `EQ_PRIMARY`), and fill color reflects the total 
number of deaths (based on the field `TOTAL_DEATHS`). A custom theme, 
`eqTheme`, is automatically applied. 
...
The function `eq_timeline` is designed to make producing these plot easier, 
but if you want to change the aesthetics arguments or theme, you can use 
the underlying geoms directly:
...
circle. The function `eq_create_label` can be used to generate a summary 
consisting of the earthquake's local location (`LocalLocation`), magnitude 
(`EQ_PRIMARY`), and total number of deaths (`TOTAL_DEATHS`).

```{r, fig.height=5, fig.width=8}
eqDataMexico <- eqDataShort %>%
